...
export async function GET() {
  try {
    ...
    return NextResponse.json({ error: status === 403 ? "Forbidden" : status === 401 ? "Unauthorized" : "Unable to load users" }, { status });
  }
}
...
export async function POST(request: Request) {
  try {
    ...
    if (!username || !fullName || password.length < 8 || !["ADMIN", "MANAGER", "EMPLOYEE"].includes(role)) {
      return NextResponse.json({ error: "Username, full name, valid role and an 8+ character password are required" }, { status: 400 });
    }
    ...
    return NextResponse.json({ error: "Unable to create user. Username may already exist." }, { status: 400 });
  }
}
